...
_LOG_PATH = Path(__file__).resolve().parent.parent / "claude-output.txt"
...
class BedrockAdapter:
    ...
    def _log_output(self, parsed: dict) -> None:
        try:
            ts = datetime.now().isoformat(timespec="seconds")
            content = parsed.get("content")
            tool_calls = parsed.get("tool_calls")
            with _LOG_PATH.open("a", encoding="utf-8") as f:
                f.write(f"\n===== {ts} | model={self.model_id} =====\n")
                if content:
                    f.write(content)
                    f.write("\n")
                if tool_calls:
                    f.write("[tool_calls] ")
                    f.write(json.dumps(tool_calls, ensure_ascii=False))
                    f.write("\n")
        except Exception as e:
            print(f"Failed to write claude-output.txt: {e}")
